...
<body class="pos-page">
                    ...
                    <div class="success-wrap text-center">
                        <form action="pos.html">
                            ...
                            <div class="d-flex align-items-center justify-content-center gap-2 flex-wrap">
                                ...
                                <a href="{{ route('billing') }}" class="btn btn-md btn-primary">Yes, Proceed</a>
                            </div>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="tax-invoice">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="invoice-user-name"><span>Invoice No: </span><span id="invoice-number"></span></div>
                            </div>
                            <div class="col-md-6">
                                <div class="invoice-user-name"><span>Customer Id: </span>#</div>
                                ...
                                <table class="table-borderless w-100 table-fit">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-end" id="receipt-discount">AED0.00</td>
                                        </tr>
                                        <tr>
    ...
    <!-- Modal  take-->
    <div class="modal fade" id="scan-payment1" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-3">
                <div class="modal-header border-0">
                    <h5 class="modal-title">Confirm Takeaway Payment</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p>Confirm UPI payment and save order?</p>
                    <button id="confirmTakeaway" class="btn btn-primary w-100">Yes, Save Order</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- take away -->
    <script>
        $("#confirmTakeaway").on("click", function() {
            let items = [];
            let totalItems = 0;
            let subtotal = 0;
            ...
            $("#cart-items tr").each(function() {
                ...
                const price = parseFloat(row.find(".qty").data("price"));
            ...
            const tax = parseFloat($("#tax-value").text()) || 0;
            const discount = parseFloat($("#discount-value").text().replace("AED", "")) || 0;
            const total = subtotal + (subtotal * tax / 100) - discount;

            // AJAX Submit
            $.ajax({
                url: "{{ route('takeaway.store') }}",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    items: items,
    ...
    <script>
        $(document).ready(function() {
            ...
            // Add product directly to the cart without order management
    ...
    <script>
        $(document).ready(function() {
            $("#submitBtn").on("click", function() {
                // Get values safely
                let subtotal = parseFloat($("#total-payable").text().replace("AED", "").trim()) || 0;
                let discount = parseFloat($("#discount-value").text().replace("AED", "").trim()) || 0;
                let taxPercentage = parseFloat($("#tax-value").text().replace("%", "").trim()) || 5; // default 5%
                let taxAmount = ((subtotal - discount) * taxPercentage) / 100;
                let totalBill = subtotal - discount;
                let sub_tot = subtotal - taxAmount;
                let receivedAmount = parseFloat($("#received-amount").val()) || 0;
                ...
                $("#cart-items tr").each(function(index) {
                    ...
                    let total = parseFloat($(this).find(".price").text().replace("AED", "").trim()) || 0;
                    ...
                    <tr>
                        ...
                        <td>${quantity}</td>
                        <td class="text-end">AED${total.toFixed(2)}</td>
                    </tr>`;
                    receiptItems.append(newRow);
                });

                // Populate receipt summary
                $("#receipt-subtotal").text(`AED${sub_tot.toFixed(2)}`);
            ...
            // Generate a random 4-digit invoice number starting with 'CS'
            let invoiceNumber = "CS" + Math.floor(1000 + Math.random() * 9000);

            // Set the invoice number in the receipt
